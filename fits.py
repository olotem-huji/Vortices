...
def hyperbola(x, a, b, c):
    # Use a mask for the condition
    mask = np.abs(x - 200) < 50
    result = (a) * (x + b)**-2 + c
    result[mask] = 100  # set value to 1 where condition is true
    return result

def parabola(x, a, b, c):
    return a * (x + b)**2 + c

def get_fit(x, y, func="half_hyperbola"):
    x_data = np.array(x)
    y_data = np.array(y)
    ...
    p0 = [0.9, 0.5 * (max(x)+min(x)), 240]  # a, b (asymptote to right of data), c
    ...
    plt.plot(x_fit, y_fit, label=f"Fit ({func})", color="blue")
    ...
    print(f"Fitted parameters for {func}: {popt}")
